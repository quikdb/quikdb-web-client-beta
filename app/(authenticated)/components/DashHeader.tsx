    ...
    <div className="flex items-center justify-between p-6 max-md:p-4 border-b border-b-[#1B1C1F]">
      ...
      <DropdownMenu>
        <DropdownMenuTrigger asChild>
          <Button
            variant="outline"
            className="bg-transparent text-gray-400 border border-gray-600 max-md:text-xs max-md:h-8"
          >
            <GlobeIcon />
            <span
              title={firstName} // Tooltip with the full name
              className="truncate"
              style={{ maxWidth: '120px', display: 'inline-block', overflow: 'hidden', textOverflow: 'ellipsis' }}
            >
              {firstName.slice(0, 12)} {/* Limit to 12 characters */}
            </span>{' '}
            Org <ChevronDown />
          </Button>
        </DropdownMenuTrigger>
        <DropdownMenuContent className="max-md:translate-x-[-50%] font-medium p-4 max-md:p-2 px-8 max-md:px-5 mt-2 ml-32 bg-[#111015] text-white border border-gray-600 border-b-[#40E39D]">
          <div className="flex flex-col gap-1 mb-5 max-md:mb-2">
            <p className="text-gradient text-lg max-md:text-xs">
              {firstName.slice(0, 12)}'s Org - 2024-11-02
            </p>
      ...
      <div className="flex items-center gap-3 text-lg max-md:ml-5">
        <div className="flex gap-2">
          ...
          <p className="max-md:text-xs">{credits} Credits</p>
        </div>
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <div className="flex gap-3 max-md:gap-0 cursor-pointer">
                ...
                <p
                  className="flex items-center gap-2 text-gray-400 max-md:text-xs"
                  title={firstName} // Tooltip with the full name
                >
                  {firstName.slice(0, 12)} <ChevronDown size={16} />
                </p>
              </div>
            </DropdownMenuTrigger>
